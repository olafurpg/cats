...
class ValidatedTests extends CatsSuite {
  ...
  checkAll("Validated[?, ?]",
           BitraverseTests[Validated].bitraverse[Option, Int, Int, Int, String, String, String])
...

  {
    ...
    checkAll("Validated[ListWrapper[String], ?]",
             SemigroupKTests[Validated[ListWrapper[String], ?]].semigroupK[Int])
  ...
  test("ap2 combines failures in order") {
    ...
    Applicative[Validated[String, ?]].ap2(Valid(plus))(Invalid("1"), Invalid("2")) should ===(
      Invalid("12"))
  }
