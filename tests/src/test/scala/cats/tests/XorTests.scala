...
class XorTests extends CatsSuite {
  ...
  checkAll(
    "Xor[ListWrapper[String], ?]", SemigroupKTests[Xor[ListWrapper[String], ?]].semigroupK[Int])
  checkAll("SemigroupK[Xor[ListWrapper[String], ?]]",
           SerializableTests.serializable(SemigroupK[Xor[ListWrapper[String], ?]]))

  {
    ...
    checkAll(
      ...
      SerializableTests.serializable(PartialOrder[ListWrapper[String] Xor ListWrapper[Int]]))
  }
  ...
  checkAll(
    "? Xor ?", BitraverseTests[Xor].bitraverse[Option, Int, Int, Int, String, String, String])
