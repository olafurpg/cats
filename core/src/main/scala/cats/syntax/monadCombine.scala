...
trait MonadCombineSyntax {
  ...
  implicit def monadCombineSyntax[F[_]: MonadCombine, G[_], A](
      fga: F[G[A]]): MonadCombineOps[F, G, A] =
    new MonadCombineOps[F, G, A](fga)

  implicit def separateSyntax[F[_]: MonadCombine, G[_, _], A, B](
      fgab: F[G[A, B]]): SeparateOps[F, G, A, B] =
