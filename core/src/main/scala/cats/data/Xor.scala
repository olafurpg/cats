...
sealed abstract class Xor[+A, +B] extends Product with Serializable {
  ...
  def compare[AA >: A, BB >: B](that: AA Xor BB)(implicit AA: Order[AA], BB: Order[BB]): Int =
    fold(
  ...
  def traverse[F[_], AA >: A, D](f: B => F[D])(implicit F: Applicative[F]): F[AA Xor D] =
    this match {
  ...
  final def combine[AA >: A, BB >: B](that: AA Xor BB)(
      implicit AA: Semigroup[AA], BB: Semigroup[BB]): AA Xor BB = this match {
    case Xor.Left(a1) =>
      that match {
...
private[data] sealed abstract class XorInstances extends XorInstances1 {
  ...
  implicit val xorBitraverse: Bitraverse[Xor] = new Bitraverse[Xor] {
    def bitraverse[G[_], A, B, C, D](fab: Xor[A, B])(f: A => G[C], g: B => G[D])(
  ...
  implicit def xorInstances[A]: Traverse[A Xor ?] with MonadError[Xor[A, ?], A] =
    new Traverse[A Xor ?] with MonadError[Xor[A, ?], A] {
      ...
      def foldRight[B, C](fa: A Xor B, lc: Eval[C])(f: (B, Eval[C]) => Eval[C]): Eval[C] =
        fa.foldRight(lc)(f)
...
private[data] sealed abstract class XorInstances1 extends XorInstances2 {
  ...
  implicit def xorPartialOrder[A : PartialOrder, B : PartialOrder]: PartialOrder[A Xor B] =
