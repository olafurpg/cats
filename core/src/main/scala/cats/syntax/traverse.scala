...
trait TraverseSyntax1 {
  implicit def traverseSyntaxU[FA](fa: FA)(
      implicit U: Unapply[Traverse, FA]): TraverseOps[U.M, U.A] =
...
trait TraverseSyntax extends TraverseSyntax1 {
  ...
  implicit def nestedTraverseSyntax[F[_]: Traverse, G[_], A](
      fga: F[G[A]]): NestedTraverseOps[F, G, A] =
