...
trait TestSettings extends Configuration with Matchers {
  ...
  lazy val checkConfiguration: PropertyCheckConfiguration = PropertyCheckConfiguration(
 ...
 */
trait CatsSuite
    extends FunSuite with Matchers with GeneratorDrivenPropertyChecks with Discipline
    with TestSettings with AllInstances with AllSyntax with TestInstances with StrictCatsEquality {
...
trait SlowCatsSuite extends CatsSuite {
  implicit override val generatorDrivenConfig: PropertyCheckConfiguration = slowCheckConfiguration
}
