...
class OneAndTests extends CatsSuite {
  ...
  checkAll("OneAnd[List, Int] with Option",
           TraverseTests[OneAnd[List, ?]].traverse[Int, Int, Int, Int, Option, Option])
  ...
  checkAll(
    "Reducible[OneAnd[List, ?]]", SerializableTests.serializable(Reducible[OneAnd[List, ?]]))

  implicit val iso = CartesianTests.Isomorphisms
    .invariant[OneAnd[ListWrapper, ?]](OneAnd.oneAndFunctor(ListWrapper.functor))
...

  {
    ...
    checkAll(
      "OneAnd[ListWrapper, Int]", FunctorTests[OneAnd[ListWrapper, ?]].functor[Int, Int, Int])
...

  {
    ...
    checkAll("SemigroupK[OneAnd[ListWrapper, A]]",
  ...
  test("reduceRight consistent with foldRight") {
    forAll { (nel: NonEmptyList[Int], f: (Int, Eval[Int]) => Eval[Int]) =>
      nel.reduceRight(f).value should ===(
        nel.tail.foldRight(nel.head)((a, b) => f(a, Now(b)).value))
    }
  }
