...
sealed abstract class Ior[+A, +B] extends Product with Serializable {
  ...
  final def onlyLeftOrRight: Option[A Xor B] =
    fold(a => Some(Xor.left(a)), b => Some(Xor.right(b)), (_, _) => None)
  ...
  final def unwrap: (A Xor B) Xor (A, B) =
    fold(a => Xor.left(Xor.left(a)), b => Xor.left(Xor.right(b)), (a, b) => Xor.right((a, b)))
