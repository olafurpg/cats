...
class ReducibleTest extends CatsSuite {
  ...
  val nevlReducible: Reducible[NEVL] = Reducible[NonEmptyList].compose[NonEmptyVector]
  checkAll("NonEmptyList compose NonEmptyVector",
           ReducibleTests(nevlReducible).reducible[Option, Int, String])
  checkAll("Reducible[NonEmptyList compose NonEmptyVector]",
