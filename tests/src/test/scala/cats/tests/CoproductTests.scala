...
class CoproductTests extends CatsSuite {
  ...
  checkAll(
    ...
    TraverseTests[Coproduct[Option, Option, ?]].traverse[Int, Int, Int, Int, Option, Option])
  checkAll("Traverse[Coproduct[Option, Option, ?]]",
...

  {
    ...
    checkAll("Coproduct[Option, Option, ?]",
             FoldableTests[Coproduct[Option, Option, ?]].foldable[Int, Int])
    checkAll("Foldable[Coproduct[Option, Option, ?]]",
             SerializableTests.serializable(Foldable[Coproduct[Option, Option, ?]]))
  }

  checkAll(
    "Coproduct[Eval, Eval, ?]", ComonadTests[Coproduct[Eval, Eval, ?]].comonad[Int, Int, Int])
  checkAll("Comonad[Coproduct[Eval, Eval, ?]]",
           SerializableTests.serializable(Comonad[Coproduct[Eval, Eval, ?]]))

  {
    implicit val coflatMap = Coproduct.coproductCoflatMap[Eval, Eval]
    checkAll("Coproduct[Eval, Eval, ?]",
             CoflatMapTests[Coproduct[Eval, Eval, ?]].coflatMap[Int, Int, Int])
    checkAll("CoflatMap[Coproduct[Eval, Eval, ?]]",
             SerializableTests.serializable(CoflatMap[Coproduct[Eval, Eval, ?]]))
  }
  ...
  implicit def showEq[A](implicit arbA: Arbitrary[A], stringEq: Eq[String]): Eq[Show[A]] =
    new Eq[Show[A]] {
  ...
  checkAll("Coproduct[Show, Show, ?]",
           ContravariantTests[Coproduct[Show, Show, ?]].contravariant[Int, Int, Int])
  checkAll("Contravariant[Coproduct[Show, Show, ?]]",
