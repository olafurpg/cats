...
private[data] sealed abstract class ValidatedInstances extends ValidatedInstances1 {
  ...
  implicit def validatedOrder[A : Order, B : Order]: Order[Validated[A, B]] =
    new Order[Validated[A, B]] {
      ...
      override def partialCompare(x: Validated[A, B], y: Validated[A, B]): Double =
        x partialCompare y
      override def eqv(x: Validated[A, B], y: Validated[A, B]): Boolean = x === y
    }

  implicit def validatedShow[A, B](implicit A: Show[A], B: Show[B]): Show[Validated[A, B]] =
    new Show[Validated[A, B]] {
      def show(f: Validated[A, B]): String = f.show
    }

  implicit val validatedBitraverse: Bitraverse[Validated] = new Bitraverse[Validated] {
    def bitraverse[G[_], A, B, C, D](fab: Validated[A, B])(f: A => G[C], g: B => G[D])(
        implicit G: Applicative[G]): G[Validated[C, D]] =
      fab match {
    ...
    def bifoldRight[A, B, C](fab: Validated[A, B], c: Eval[C])(
...
private[data] sealed abstract class ValidatedInstances1 extends ValidatedInstances2 {
  ...
  implicit def validatedPartialOrder[A : PartialOrder, B : PartialOrder]: PartialOrder[Validated[
      A, B]] =
